...
namespace CitizenManagement_EntityFramework.DAO
{
    public class PeopleMarriageDAO
    {
        ...
        {
            string conn = String.Format("EXEC PROC_RegisterMarriage '{0}', '{1}', '{2}', N'{3}', '{4}', N'{5}'", hn.Mahn, hn.Macdchong, hn.Macdvo, hn.Loai, hn.Ngaydangky, hn.Trangthai);
            return DBConnection.Instance.Execute(conn);
        }
        ...
        {
            DataTable dataTable = new DataTable();
            string conn = string.Format("SELECT * FROM [V_DataFmailyNotConfirm]");
            dataTable = DBConnection.Instance.GetDataTable(conn);
            return dataTable;
        }
        ...
        {
            DataTable dataTable = new DataTable();
            string conn = string.Format("SELECT * FROM [V_DataFmailyNotBrowse]");
            dataTable = DBConnection.Instance.GetDataTable(conn);
            return dataTable;
        }
