...
namespace CitizenManagement_EntityFramework.DAO
{
    public class MailDAO
    {
        ...
        {
            string sqlStr = string.Format("DELETE FROM Mails WHERE MaMail = '{0}' AND (NguoiGui = '{1}' or NguoiNhan = '{1}')", maMail, CurrentUser.Instance.CurrentCitizen.Macd);
            DBConnection.Instance.Execute(sqlStr);
        }
        ...
        {
            string sqlStr = string.Format("SELECT * FROM {2} ('{1}') WHERE {3} LIKE '%{0}%'", name, CurrentUser.Instance.CurrentCitizen.Macd, function, searchingPerson);
            return DBConnection.Instance.GetDataTable(sqlStr);
        }

        public DataTable getMailsByDayOfCitizen(string searchingDay, string function)
        {
            string sqlStr = string.Format("SELECT * FROM {2} ('{1}') WHERE Ngay like '%{0}%'", searchingDay, CurrentUser.Instance.CurrentCitizen.Macd, function);
            return DBConnection.Instance.GetDataTable(sqlStr);
        }

        public DataTable getMailsByIDOfCitizen(string ID, string function, string searchingIDPerson)
        {
            string sqlStr = string.Format("SELECT * FROM {2} ('{1}') WHERE {3} LIKE '%{0}%'", ID, CurrentUser.Instance.CurrentCitizen.Macd, function, searchingIDPerson);
            return DBConnection.Instance.GetDataTable(sqlStr);
        }
